...
OP_ROUTES = yaml.safe_load(
    open(os.path.join(CONFIG_DIR, "admin", "routes.yaml"))
)  # Operator listener Flask routes
C2_PROFILE = MaleableProfile.from_yaml(
    open(DEFAULT_C2_PROFILE).read()
)  # C2 listener Flask routes
