...
int notmain(int argc, char *argv[])
{
    if (argc != 2)
    {
        DEBUG_PRINTF("Usage: %s <path to PE>\n", argv[0]);
        return 0;
    }
    DWORD dwFileSize = 0;
    ...
    DEBUG_PRINTF("Lets get ready to Load!\n");
    ...
    DEBUG_PRINTF("[+] The Memory mapped PE will be %d bytes!\n", dwImageSize);
    ...
    {
        DEBUG_PRINTF("Failed to allocate memory because of %d\n", GetLastError());
        return 0;
    }
    ...
    for (DWORD i = 0; i < dwSections; i++)
    {
        ...
        DEBUG_PRINTF("Copying section %s\n", sections[i].Name);
    ...
    DEBUG_PRINTF("[!] Hooray! WE memory mapped the PE!\n");
    ...
    for (int i = 0; lpImageDescriptor[i].FirstThunk != 0; i++)
    {
        ...
        if (hModule == NULL)
        {
            DEBUG_PRINTF("Failed to Load Library Because of %d\n", GetLastError());
            return -1;
        }
        DEBUG_PRINTF("Loaded %s as %p\n", dllName, (void *)hModule);
        ...
        for (int j = 0; lookup_table[j].u1.AddressOfData != 0; j++)
        {
            ...
            if (lpFunction == NULL)
            {
                DEBUG_PRINTF("Failed to Load function Because of %d\n", GetLastError());
                return -1;
            }
    ...
    {
        // nothing to be done :-)
        DEBUG_PRINTF("[+] We got our prefered base Addres or there are no relocations. Nothing to be done!\n");
    }
    else
    {
        DEBUG_PRINTF("[+] We need to handle Relocations: Shifted Base by %p\n", deltaBaseAddr);
        DEBUG_PRINTF("Reloc RVA: %lu\n", relocRVA);
        relocationBlock *relocation = (relocationBlock *)(lpBaseAddres + relocRVA);
        while (relocation->dwPageRVA > 0)
        {
            // n_relocation = dwBlockSize - 8) //2
            DEBUG_PRINTF("dwBlockSize: %lu\n", relocation->dwBlockSize);
            ...
            DEBUG_PRINTF("|__[+] There are %d relocations to perform\n", dwNRelocs);
            for (DWORD i = 0; i < dwNRelocs; i++)
            {
                ...
                DEBUG_PRINTF("|__[+] perfomring Relocation: %lu, %d\n", dwRelocType, intOffset);
                ULONGLONG *lpRelocVA = NULL;
                switch (dwRelocType)
                {
                case IMAGE_REL_BASED_ABSOLUTE:
                    DEBUG_PRINTF("|__[+] IMAGE_REL_BASED_ABSOLUTE ---> nothing to be done!\n");
                    break;
                case IMAGE_REL_BASED_DIR64:
                    ...
                    *lpRelocVA = *lpRelocVA + deltaBaseAddr;
                    break;
                default:
                    DEBUG_PRINTF("|__[!] Unrecongized relocation type! %lu\n", dwRelocType);
                    break;
                }
                // offset
            }
            DEBUG_PRINTF("Updating Relocation Size: %lu\n", relocation->dwBlockSize);
    ...
    if (lpNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_TLS].Size)
    {
        ...
        while (*callback)
        {
            DEBUG_PRINTF("[+] TLS callback at %p\n", (void *)callback);
